...
def create_project():
    ...
    new_project = {
        ...
        'stakeholders': {
            'users': [email.strip() for email in request.form.get('users', '').split(',') if email.strip()],
            'developers': [email.strip() for email in request.form.get('developers', '').split(',') if email.strip()],
            'management': [email.strip() for email in request.form.get('management', '').split(',') if email.strip()]
        },
        'recent_updates': [update.strip() for update in request.form.get('recent_updates', '').split('\n') if update.strip()],
        'upcoming_milestones': [],
        'comms_history': [],
        'comms_plan': {
            'generated_date': None,
...
def edit_project(project_id):
    ...
    for project in data['projects']:
        if project['id'] == project_id:
            ...
            project['business_value'] = request.form.get('business_value', project['business_value'])
            project['current_phase'] = request.form.get('current_phase', project['current_phase'])
            project['expected_launch'] = request.form.get('expected_launch', project['expected_launch'])
            ...
            if updates_text:
                project['recent_updates'] = [u.strip() for u in updates_text.split('\n') if u.strip()]
...
def send_email():
    ...
    try:
        ...
        if result.get('success'):
            return jsonify({
                'success': True,
                'message': 'Email marked as sent and history updated'
            })
        else:
            return jsonify({
                'success': False,
...
def add_manual_communication(project_id):
    ...
    try:
        comm_data = {
            'id': f"comm_{str(uuid.uuid4())[:8]}",
            'date_sent': request.form.get('date_sent', datetime.now().strftime('%Y-%m-%d')),
            'type': request.form.get('type', 'status_update'),
            'audience': request.form.get('audience', 'users'),
            'subject': request.form.get('subject', ''),
            'summary': request.form.get('summary', ''),
            'key_messages': [msg.strip() for msg in request.form.get('key_messages', '').split('\n') if msg.strip()],
            'sent_to': [email.strip() for email in request.form.get('sent_to', '').split(',') if email.strip()]
        }
